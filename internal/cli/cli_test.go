...
func TestInitAndValidate(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(root, ".arbor", "arbor.yaml")); err != nil {
		t.Fatal(err)
	}
	output.Reset()
	command = New(Options{Out: &output, ErrOut: &output, Dir: root})
...
func TestNewRequestAndScenario(t *testing.T) {
	...
	for _, path := range []string{"collections/users/get.yaml", "scenarios/users/smoke.yaml"} {
		if _, err := os.Stat(filepath.Join(workspace.DataDir(root), path)); err != nil {
...
func TestNewCollectionListAndDescribe(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(workspace.DataDir(root), "collections", "users", "collection.yaml")); err != nil {
...
func TestContextIncludesAgentSafeWorkspaceSummary(t *testing.T) {
	root := filepath.Join(t.TempDir(), "demo")
	var output bytes.Buffer
	run := func(args ...string) {
		t.Helper()
	...
	run("new", "request", "users.get", "--url", "{{base_url}}/users/1")
	run("context")
	for _, expected := range []string{"\"schemaVersion\":1", "\"definitionSource\":\"filesystem\"", "users.get", "\"file\":\".arbor/collections/users/get.yaml\"", "arbor validate"} {
		if !strings.Contains(output.String(), expected) {
			t.Fatalf("context output missing %q: %s", expected, output.String())
		}
	}
}

func TestSkillInstallStatusAndProjectInit(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	root := filepath.Join(t.TempDir(), "demo")
	var output bytes.Buffer
	...
	run("skill", "install", "--agent", "all")
	for _, path := range []string{
		filepath.Join(home, ".codex", "skills", "arbor-api-authoring", "SKILL.md"),
		filepath.Join(home, ".agents", "skills", "arbor-api-authoring", "SKILL.md"),
		filepath.Join(home, ".claude", "skills", "arbor-api-authoring", "SKILL.md"),
	} {
		if _, err := os.Stat(path); err != nil {
			t.Fatalf("skill file missing at %s: %v", path, err)
		}
	}
	run("skill", "uninstall", "--agent", "codex")
	if _, err := os.Stat(filepath.Join(home, ".codex", "skills", "arbor-api-authoring")); !os.IsNotExist(err) {
		t.Fatalf("owned Codex skill was not removed: %v", err)
	}
	run("skill", "init")
	if _, err := os.Stat(filepath.Join(root, "AGENTS.md")); err != nil {
		t.Fatalf("project instructions missing: %v", err)
	}
	run("skill", "init")
	if !strings.Contains(output.String(), "AGENTS.md already exists") || !strings.Contains(output.String(), "# Arbor agent instructions") {
		t.Fatalf("existing AGENTS.md guidance = %q", output.String())
	}
}
...
func TestDescribeIncludesFiles(t *testing.T) {
	...
	run("init", "--name", "Demo")
	reqPath := filepath.Join(workspace.DataDir(root), "collections", "up", "post.yaml")
